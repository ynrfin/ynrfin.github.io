...
We need to make it available on the HTML. Use `x-data` from alpine.js to get
the produk list above
...
`x-data` will make all data and function(behaviour) inside `theData()` function
available inside the div scope
...
`x-for` --> will iterate the `produk` property inside that available from the
`x-data` scope
...
because creating template means that we need to change the HTML and CSS
constantly.
Previously there are only full blown templating engine included on a web
framework
